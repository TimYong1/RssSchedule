    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < LETTERS.length; i++) {
            ...
            if(mTouchIndex == i){
                ...
                float radius =  mCellHeight/2;
                float circleY = y-mPaint.measureText(text)/2.0F;
                float circleX = Float.valueOf(mCellWidth)/2;
                canvas.drawCircle(circleX,circleY,radius,mPaint);
            }

            mPaint.getTextBounds(text, 0, text.length(), bounds);
            mPaint.setColor(mTouchIndex == i ? Color.WHITE : Color.GRAY);//被选择到的字母变成灰色
...
//        invalidate();//重新调用onDraw方法实现选中的字母更改颜色
    ...
    public void setSelectIndex(String text){
        for (int i = 0; i < LETTERS.length; i++) {
            if(LETTERS[i].equals(text)){
                mTouchIndex = i;
                invalidate();
                return;
            }
