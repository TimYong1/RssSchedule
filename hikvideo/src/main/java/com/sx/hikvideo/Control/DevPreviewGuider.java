package com.sx.hikvideo.Control;

import android.util.Log;
import android.view.SurfaceHolder;

import com.hikvision.netsdk.RealPlayCallBack;
import com.sx.hikvideo.jna.HCNetSDKJNAInstance;
...
public class DevPreviewGuider {
    ...
    //预览接口
    public int RealPlay_V40_jni(int iUserID, NET_DVR_PREVIEWINFO struPlayInfo, RealPlayCallBack playCallBack) {
        if (iUserID < 0) {
            Log.e("SimpleDemo", "RealPlay_V40_jni failed with error param");
            return -1;
        }
        int iRet = HCNetSDK.getInstance().NET_DVR_RealPlay_V40(iUserID, struPlayInfo, playCallBack);
        if(iRet < 0)
        {
            return -1;
        }
